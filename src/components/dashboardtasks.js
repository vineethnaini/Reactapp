...
class DashboardTasks extends React.Component{
    ...
    render(){
        return(
            <div>
              {
                this.props.gottask && <Table striped bordered hover size="sm">
                <thead>{
                  <tr>
                    <th>TITLE</th>
                    <th>DESCRIPTION</th>
                ...
                <tbody>{
                  ...
                  <tr key = {index.id}>
                    ...
                    {index.status === "assigned" && <td>
                  <Button variant="primary" onClick= {()=>this.handleStarting(index.id)}>Start</Button>
                  </td>}
                    {index.status === "in_progress" && <td>
                  <Button variant="primary" onClick= {()=>this.handleCompleted(index.id)}>Complete</Button>
                  </td>
                    }
                  </tr>
                  )
