...
class filterUser extends React.Component {
  ...
  componentDidMount(){
    this.props.dispatch(userActions.filterUser(this.state.name,this.state.email,this.state.role,this.state.createdBy,this.state.currentPage));
  }
  componentDidUpdate(){
  ...
  prevPage() {
    if(this.state.currentPage > 1){
      ...
      this.props.dispatch(userActions.filterUser(name,email,role,createdBy,currentPage-1))
  ...
  handleRoleChange(id) {
    this.props.dispatch(userActions.roleChange(id,this.state.name,this.state.role,this.state.email,this.state.createdBy));
  }
  handleSubmit(event) {
    alert('input details' +this.state.name + this.state.email + this.state.role + this.state.createdBy );
    ...
    render() {
      console.log(this.state)
        ...
        <Form onSubmit={this.handleSubmit}>
          <Form.Row>
            ...
            <Form.Group as={Col} controlId="formGridState">
              ...
              <Form.Control as = "select" name = "role" onChange={this.handleInputChange2}>
            ...
            <Form.Group as={Col} controlId="formGridText">
              ...
              <Form.Control type="text" name = "createdBy" value={this.state.createdBy} onChange={this.handleInputChange2} placeholder="Enter created by" />
            </Form.Group>
          </Form.Row>
          ...
          <div>
            {this.props.filtered && <Table striped bordered hover size="sm">
          <thead>{
            <tr>
              <th>NAME</th>
              <th>ROLE</th>
              {document.cookie.split(";")[1].split("=")[1] === 'admin'  && <th>ACTION</th>}
            </tr>
          }
          </thead>
          <tbody>{
            this.props.users.data.map((index)=>(
              <tr key = {index.id}>
                ...
                {document.cookie.split(";")[1].split("=")[1] === 'admin' && index.roles === "normal" && <td>
                  <Button variant="danger" onClick= {()=>this.handleDeleteUser(index.id)}>Delete</Button>
                  <Button variant="outline-primary" onClick= {()=>this.handleRoleChange(index.id)}>MakeAdmin</Button>
                  <Button variant="outline-primary" ><Link to={"/users/"+index.id} onClick= {()=>this.handleUserProfile(index.id)}>Profile</Link></Button>
                </td>
                }
                {document.cookie.split(";")[1].split("=")[1] === 'admin' && index.roles === "admin" && <td>
        ...
        {this.props.filtered && <Button onClick= {() => this.prevPage()}>Prev</Button>}
        {this.props.filtered && <Button onClick= {() => this.nextPage(this.props.users.last_page)}>Next</Button>}
        </div>
        </div>
        )
      // else {
